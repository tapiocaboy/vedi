/**
 * The Western Natal Chart modal — Sun/Moon/Rising, the ten planets, and the
 * twelve houses, click-to-expand. Presentational structure mirrors
 * `Natal/NatalChartModal.tsx` (line/section/tone shapes are intentionally
 * identical) but stays a separate component so the Vedic modal is untouched.
 */
...
import { buildWesternNatalReport, type WesternNatalLine, type WesternNatalSection, type Tone } from '../../lib/core/western/natal';
...
const TONE_TEXT: Record<Tone, string> = {
  good: 'text-emerald-300', bad: 'text-rose-300', neutral: 'text-white/55', info: 'text-violet-300',
};
...
const TONE_BULLET: Record<Tone, string> = {
  good: 'text-emerald-400', bad: 'text-rose-400', neutral: 'text-violet-400', info: 'text-violet-400',
};

function LineIcon({ icon }: { icon: string }) {
  const glyph = westernPlanetGlyph(icon); // 'SUN'…'PLUTO', 'ASCENDANT', or a house number (falls through unchanged)
  return (
...
const Section: React.FC<{ s: WesternNatalSection }> = ({ s }) => (
...
const LineRow: React.FC<{ line: WesternNatalLine; open: boolean; onToggle: () => void }> = ({ line, open, onToggle }) => (
...
const GROUP_LABEL: Record<WesternNatalLine['group'], string> = {
  bigthree: 'Sun, Moon & Rising',
  planet: 'The Ten Planets',
  house: 'The Twelve Houses',
};

const Body: React.FC<{ birthData: BirthData }> = ({ birthData }) => {
  const { lang, t } = useLang();
  const [openId, setOpenId] = useState<string | null>('sun');
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-white/45 text-sm py-16 justify-center">
        <Loader2 className="w-4 h-4 animate-spin" /> {t('western.natal.computing')}
      </div>
    );
  }
  if (error || !data) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border p-3.5" style={{ borderColor: 'rgba(var(--c-accent-rgb),0.20)', background: 'rgba(var(--c-accent-rgb),0.05)' }}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[11px] font-semibold uppercase tracking-wider text-white">
            {data.meta.sunSign} Sun · {data.meta.moonSign} Moon · {data.meta.risingSign} Rising
          </span>
        </div>
        <p className="text-[12px] text-white/70 leading-relaxed">{t('western.natal.subtitle')}</p>
      </div>
      ...
      <p className="text-[10px] text-white/30 text-center pt-1">{t('western.natal.disclaimer')}</p>
    </div>
  );
};

export const WesternNatalModal: React.FC<Props> = ({ visible, onClose, birthData }) => {
  const { t } = useLang();
  return (
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 p-4 sm:p-5 border-b border-white/8 shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-semibold text-white">{t('western.natal.title')}</h3>
                <p className="text-[11px] text-white/45 truncate">{t('western.natal.subtitle')}</p>
              </div>
